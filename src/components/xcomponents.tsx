...
const Dropdown: React.FC<DropdownProps> = ({ label, options, onSelect }) => {
    ...
    return (
        <div ref={dropdownRef} className="relative">
            <button
                className="group font-semibold min-w-3xs min-h-14 h-max flex items-center justify-between px-4 py-2 bg-(--secondary-color) text-(--primary-color) rounded-lg hover:opacity-75 transition-all focus:outline-none cursor-pointer"
            ...
            {isOpen && (
                <div className="absolute min-w-3xs gap-4 flex flex-col p-2 max-h-40 overflow-y-auto overflow-x-hidden bg-(--background-color)/90 backdrop-blur-sm mt-2 w-max  rounded-lg shadow-lg z-10">
